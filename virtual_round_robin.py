...
if count == 0 or count < 0:
    ...
    while True:
        if not auxiliary_queue.isEmpty():
            running_process = auxiliary_queue.dequeue()
            ...
            if len(waiting_queue) > 0:
                ...
                for every_process in waiting_queue:
                    ...
                    if every_process["return_time"] <= running_time:
                        auxiliary_queue.enqueue(waiting_queue.pop(index))
                    index += 1
            else:
                break
